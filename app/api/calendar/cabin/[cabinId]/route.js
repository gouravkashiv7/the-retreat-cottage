...
export async function GET(request, { params }) {
  ...
  if (!cabinId) {
    return new Response("Missing cabinId", { status: 400 });
  }

  try {
    // 1. Fetch live availability including internal DB and external OTAs
    const res = await fetch(
      `https://kckngulhvwryekywvutn.supabase.co/functions/v1/get-live-availability?cabinId=${cabinId}`,
    );
    ...
    internalBookings.forEach((booking, index) => {
      ...
      // iCal end dates for all day events are exclusive, so if endDate is checkout, it's correct
      const end = format(new Date(booking.endDate), "yyyyMMdd");

      icalContent.push(
        "BEGIN:VEVENT",
        `UID:booking-cabin-${cabinId}-${start}-${index}@theretreatcottage.com`,
        `DTSTAMP:${now}`,
        `DTSTART;VALUE=DATE:${start}`,
        `DTEND;VALUE=DATE:${end}`,
        "SUMMARY:Reserved (The Retreat Cottage Cabin)",
        "STATUS:CONFIRMED",
        "END:VEVENT",
      );
    });
    ...
    return new Response(icalContent.join("\r\n"), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="cabin-${cabinId}-availability.ics"`,
